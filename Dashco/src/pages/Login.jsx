  ...
  const handleSubmit = useCallback(async (event) => {
    ...
    try {
      ...
      if (data?.user && data?.session) {
        ...
        // Create user profile in public.users if it doesn't exist (non-blocking)
        supabase
          .from("users")
          .upsert({
            id: data.user.id,
        ...
        // Redirect to dashboard - session is already set
        navigate("/dashboard");
  ...
  const handleGoogleSignIn = useCallback(async () => {
    ...
    try {
      ...
      if (authError) {
        ...
        // If it's a browser security error, provide additional guidance
        if (errorMessage.includes('browser') || errorMessage.includes('secure')) {
          console.warn('Google OAuth browser security issue detected. User may need to:', {
            action: 'Add test user in Google Cloud Console',
            url: 'https://console.cloud.google.com/apis/credentials/consent',
            note: 'Use a regular browser (Chrome/Firefox) instead of automated testing tools',
          });
        }
      } else if (data?.url) {
        // OAuth flow initiated successfully - redirect will happen
        // Don't set loading to false, as redirect is about to happen
        console.log('Google OAuth initiated, redirecting...');
        // Note: The redirect happens automatically via window.location
        // We don't need to set loading to false here as the page will redirect
      } else {
        // No URL and no error - unexpected state
        setError("Failed to initiate Google sign-in. Please try again.");
        setLoading(false);
      }
    } catch (err) {
      console.error('Google sign-in exception:', err);
      setError(err.message || "An unexpected error occurred. Please try again or use email/password login.");
      setLoading(false);
    }
  }, []);
        ...
        <div className="max-w-[28rem] mx-auto w-full px-6 sm:px-8 pb-8">
          ...
          {error && (
            <div className="mt-4 p-4 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
              ...
              {(error.includes('browser') || error.includes('secure') || error.includes('test user')) && (
                <div className="text-xs text-red-500 dark:text-red-400 mt-2 space-y-1">
                  <p><strong>Quick Fix:</strong></p>
                  <ol className="list-decimal list-inside space-y-1 ml-2">
                    <li>Use a regular browser (Chrome or Firefox)</li>
                    <li>Go to <a href="https://console.cloud.google.com/apis/credentials/consent" target="_blank" rel="noopener noreferrer" className="underline">Google Cloud Console</a></li>
                    <li>Add your email as a test user in the OAuth consent screen</li>
                    <li>Wait 1-2 minutes and try again</li>
                  </ol>
                  <p className="mt-2 text-gray-600 dark:text-gray-400">
                    Or use <strong>email/password login</strong> below for testing.
                  </p>
                </div>
              )}
              {(error.includes('Chrome') || error.includes('cookie') || error.includes('storage') || error.includes('Third-party') || error.includes('privacy')) && (
                <div className="text-xs text-red-500 dark:text-red-400 mt-2 space-y-1">
                  <p><strong>Chrome Privacy Settings Fix:</strong></p>
                  <ol className="list-decimal list-inside space-y-1 ml-2">
                    <li>Open Chrome Settings → Privacy and security → Cookies and other site data</li>
                    <li>Enable "Allow third-party cookies" (or add localhost as exception)</li>
                    <li>Ensure "Block third-party cookies" is disabled for localhost</li>
                    <li>Make sure you're not in Incognito/Private mode</li>
                    <li>Clear cookies for localhost:5173 and try again</li>
                  </ol>
                  <p className="mt-2 text-gray-600 dark:text-gray-400">
                    Alternative: Use <strong>Firefox</strong> which has more lenient cookie policies, or use <strong>email/password login</strong>.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5 mb-0 mt-8 w-full">
            ...
            <button
              type="button"
              disabled
              className="w-full inline-flex items-center justify-center gap-2 sm:gap-3 h-11 px-4 sm:px-7 py-3 rounded-lg bg-[#f2f4f7] dark:bg-white/5 text-[#344054] dark:text-white/90 text-sm font-normal border-none cursor-not-allowed transition-all duration-200 whitespace-nowrap opacity-50 leading-5"
            >
              <svg
                width="20"
                height="20"
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-0 mt-0">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-11 px-4 py-3 text-sm font-medium leading-5 rounded-lg bg-[#465FFF] hover:bg-[#3641F5] text-white inline-flex items-center justify-center transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
